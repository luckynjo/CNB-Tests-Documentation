...
import {PMATTrials} from '../trials/PMATTrials.js';
import {BeginPage} from '../components/BeginPage.js';
import banner from '../assets/pmat/banner.png';
...
export default class PMAT extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      index: 0,
    ...
    this.feedback_correct = content[2];
    this.feedback_incorrect = content[3];
  ...
  render()
  {
    ...
    {
      return <TitlePage banner={banner} content={JSON.parse(timeline_object.content)} banner_width={600} theme={"light"} continue_button_text={this.continue_button_text} onClick={this.next} {...this.props.test}/>
    }
    ...
    {
      return <PMATTrials base_url={this.props.base_url} section="practice" buttons={JSON.parse(timeline_object.content)} trials={this.props.practice_trials} images={this.images} onTrialsComplete={this.next} feedback_correct={this.feedback_correct} feedback_incorrect={this.feedback_incorrect}/>
    }
    else if(section_title.match(TEST_REGEX)){
      return <PMATTrials base_url={this.props.base_url} section="test" buttons={JSON.parse(timeline_object.content)} trials={this.props.test_trials} images={this.images} onTrialsComplete={this.onTrialsComplete}/>
    }
    else
    {

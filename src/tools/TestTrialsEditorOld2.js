...
export class TestTrialsEditor extends React.Component
{
  ...
  constructor(props)
  {
    ...
    this.setSearchDetailsHoverStyle = this.setSearchDetailsHoverStyle.bind(this);
  ...
  handleToggle = (e) => {
    ...
    else if(e.target.tagName == "path" && e.target.parentElement.parentElement.id == "searchbarContainer"){
      const searchValue = e.target.parentElement.parentElement
      this.filterData(trialsDummyData, this.state.searchInput);
    }
    else if(e.target.tagName == "svg" && e.target.parentElement.id == "searchbarContainer"){
      this.filterData(trialsDummyData, this.state.searchInput);
    }
    else if(e.key == "Enter"){
      e.preventDefault();
  ...
  searchQueryAction(e, action){
    let parentElm = e.target.parentElement.parentElement.childNodes[1];
    ...
    if(keywordOption != "" && operatorOption != "" && valueOption != ""){
  ...
  setSearchDetailsHoverStyle(newData)
  {
    this.setState((prevState, props) => {
      return {searchDetailsHoverStyle: newData}
    });
  }
  ...
  filterData(data, currentSearchInput) {
    ...
    console.log("SEARCHING::: ", inputValue, query);
    ...
    (
      ...
      <div id="toolsbar" style={{display : 'flex', gap: '10px'}}>
          ...
          <div id="searchContainer" style={{display: "flex", position: "relative"}}>
            ...
            {/* {(this.state.searchToggleMode)? 
              <div id="searchbarContainer" style={{backgroundColor: "#2e3e51", display: (this.state.toggleDataView) ? "none" : "flex", 
                flex: 1, alignItems: "center", borderRadius: "8px", border: "1px solid black", 
                padding: "3px 10px 3px 10px", gap: "10px", cursor: 'pointer', transition: "transform 0.25s linear", position: "relative", 
                zIndex: 10}}>
                <select name="search_kywd" id="search_kywd">
                ...
                <input type="text" name="search_val" id="search_val" placeholder="Type A Value"></input>
              </div>
            :
              <div id="searchbarContainer" style={{backgroundColor: "#2e3e51", display: (this.state.toggleDataView) ? "none" : "flex", 
                flex: 1, alignItems: "center", width: "310px", borderRadius: "8px", border: "1px solid black", 
                ...
                <div id="searchModeWrapper" style={{display: (this.state.searchToggleMode)? "block" : "none", maxHeight: (this.state.searchToggleMode)? "300px" : 0, padding: (this.state.searchToggleMode)? "15px" : 0, opacity: (this.state.searchToggleMode)? 1 : 0, transition: "max-height 0.5s, padding 0.3s, opacity 0.3s", overflow: "hidden", backgroundColor: "#2e3e51", boxShadow: "0 0 0 2px #ffffff, 0 0 0 5px #4A6572", alignItems: "center", borderRadius: "8px", border: "1px solid black"}}>
                  <div id="searchModeContainer" style={{display: "flex", flexDirection: "column", gap: "10px", cursor: 'pointer', color: "white", overflow: "auto", height: "100%", padding: "10px 15px"}}>
                    ...
                    <div id="searchModeOptions" style={{display: "flex", gap: "10px", justifyContent: "space-between", marginTop: "20px"}}>
                      <div id="addQuery" style={{color: '#ffffff', backgroundColor: "rgb(29, 39, 52)", borderRadius: "8px", border: "1px solid white", padding: "5px"}} onClick={this.handleClick}>Add Query</div>
                      <div id="searchTable" style={{color: '#ffffff', backgroundColor: "rgb(29, 39, 52)", borderRadius: "8px", border: "1px solid white", padding: "5px"}} onClick={this.handleClick}>Search Table</div>
                      <div id="clearAll" style={{color: '#ffffff', backgroundColor: "rgb(29, 39, 52)", borderRadius: "8px", border: "1px solid white", padding: "5px"}} onClick={this.handleClick}>Clear All</div>
                    </div>
                  </div>
                </div>
          ...
          <button id="toggleResetSearchBtn" 
                  style={{
                    display: (this.state.toggleResetSearch)? "block" : "none",
                    padding: '12px 24px',
                    fontSize: '20px',
                    fontWeight: '600',
                    color: '#ffffff', // White text for contrast on dark button
                    backgroundColor: '#2C3E50', // Solid dark blue-gray background
                    borderRadius: '8px',
                    cursor: 'pointer',
                    border: 'none',
                    fontFamily: 'Inter, sans-serif',
                  ...
                  onClick={this.handleToggle}>Reset Search
          </button>
        </div>
      </div>

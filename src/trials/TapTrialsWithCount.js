...
export class TapTrialsWithCount extends React.Component{
  constructor(props)
  {
    super(props);
    this.state = {
  ...
  onPracticeResponse(e)
  {
    if(e.keyCode === 32)
    {
      ...
      //   this.updateGoCount();
      // }
      if((goCount === 0) || (this.state.stimulus != this.props.stopText)){
        this.updateGoCount();
      }
    }
  }
  ...
  render()
  {
    ...
    return (
      ...
      {(this.state.stimulus !== this.props.stopText) ? <div style={{left: "50%", textAlign: "center", border: "2px solid white", color: "white"}}>{this.state.goCount}</div> : ""}
      </div>
    );
  }
}
